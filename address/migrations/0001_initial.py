# Generated by Django 3.1.5 on 2021-01-16 05:17

import datetime
from django.conf import settings
import django.contrib.gis.db.models.fields
import django.contrib.gis.geos.point
from django.db import migrations, models
import django.db.models.deletion
import phonenumber_field.modelfields


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Address',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('date_created', models.DateTimeField(default=datetime.datetime.now, editable=False)),
                ('nickname', models.CharField(max_length=64)),
                ('is_primary', models.BooleanField(default=False)),
                ('address_type', models.CharField(choices=[('Home', 'Home'), ('Billing', 'Billing'), ('Pickup', 'Pickup'), ('Delivery', 'Delivery'), ('Other', 'Other'), ('Business', 'Business')], max_length=16)),
                ('contact_name', models.CharField(max_length=128)),
                ('contact_phone', phonenumber_field.modelfields.PhoneNumberField(max_length=128, region=None)),
                ('address', models.CharField(max_length=64)),
                ('address_other', models.CharField(blank=True, max_length=16)),
                ('locality_disctrict', models.CharField(blank=True, max_length=64)),
                ('place_city', models.CharField(max_length=64)),
                ('district_prefectures', models.CharField(blank=True, max_length=64)),
                ('region_state', models.CharField(blank=True, max_length=64)),
                ('postcode', models.CharField(max_length=12)),
                ('country', models.CharField(max_length=64)),
                ('latitude', models.FloatField(default=42.95)),
                ('longitude', models.FloatField(default=-83.64)),
                ('location', django.contrib.gis.db.models.fields.PointField(default=django.contrib.gis.geos.point.Point(0.0, 0.0), srid=4326)),
                ('relevance', models.FloatField(blank=True, default=0)),
                ('accuracy', models.CharField(blank=True, max_length=16)),
                ('user_fk', models.ForeignKey(default=1, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
