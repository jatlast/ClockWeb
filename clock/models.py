...
class Clocktypes(models.Model):
    CLOCK_TYPE_CHOICES=[
        ...
        ('Desk', 'Desk'),
        ('Dial', 'Dial'),
        ('Drop Trunk/Schoolhouse', 'Drop Trunk/Schoolhouse'),
        ('Figural', 'Figural'),
...
#    customer_fk = models.ForeignKey(Customer, on_delete=models.CASCADE, default=new_uuid)
    ...
    clock_type_fk = models.ForeignKey(Clocktypes, on_delete=models.CASCADE)

    # nickname
    nickname = models.CharField(blank=False, max_length=64, default='', help_text='Please choose a Nickname to identify this clock')
    # date_created - autopopulated
    date_created = models.DateTimeField(default=datetime.now, editable=False)
    # clock_type - Advertising, Atmos, Banjo, Kitchen, etc
    #   When displaying use: https://www.antiqueclockspriceguide.com/clocktyperesults.php?t=clocktype.clock_type
...
#    clock_type = models.CharField(blank=False, max_length=32, choices=Clocktype.CLOCK_TYPE_CHOICES, default='Longcase/Grandfather')
    ...
    choices_are_locked = models.BooleanField(blank=False, default=False, help_text="(Note: Repairer should select this checkbox after confirming the clock's information to prevent the customer from making incorrect changes)")
    # image_# - Five associated pictures
